<?php
    ...
    if(strtolower($status) == "reject"){
        ...
        if($objConnect){
            //check having work
            $SQL = "select work_no from fttx_workorder_pat_test where REF_NO = '$id' and P_STATUS='Submit' ";
            ...
            if(sizeof($res)==0){
                ...
                if($objConnect){
                    ...
                    if($res[0]["WORK_NO"] == ""){
                        ...
                        $SQL = "UPDATE fttx_workorder_pat_test SET P_STATUS='',VERIFY_EXPORT_SPLITTER1='',CAT_SUBMIT_DATE='',CAT_COMPLETE_DATE='',OTDR_FILE_PATH=''  where WORK_NO = '$worn' and lot is null";
                        $objParse = oci_parse($objConnect, $SQL);
                        oci_execute ($objParse,OCI_DEFAULT);
                        
                        $SQL = "UPDATE fttx_inventory_temp_test SET P_STATUS='' where WORK_NO = '$worn' and lot is null";
                        $objParse = oci_parse($objConnect, $SQL);
                        oci_execute ($objParse,OCI_DEFAULT);
                
                        $SQL = "insert into fttx_log_test(name,action,work_no,action_date) values ('ATTS', 'Reject', '$worn', SYSDATE) ";
                        $objParse = oci_parse($objConnect, $SQL);
                        oci_execute ($objParse,OCI_DEFAULT);
            ...
            if(sizeof($res)==0){
                ...
                if($objConnect){
                    ...
                    if(sizeof($res)==0){
                        ...
                        $SQL = "UPDATE fttx_workorder_pat_test SET P_STATUS='Approve' where WORK_NO = '$worn' and lot is null";
                        $objParse = oci_parse($objConnect, $SQL);
                        oci_execute ($objParse,OCI_DEFAULT);
                        
                        $SQL = "UPDATE fttx_inventory_temp_test SET P_STATUS='Approve' where WORK_NO = '$worn' and lot is null";
                        $objParse = oci_parse($objConnect, $SQL);
                        oci_execute ($objParse,OCI_DEFAULT);
                
                        $SQL = "insert into fttx_log_test(name,action,work_no,action_date) values ('ATTS', 'Approve', '$worn', SYSDATE) ";
                        $objParse = oci_parse($objConnect, $SQL);
                        oci_execute ($objParse,OCI_DEFAULT);
