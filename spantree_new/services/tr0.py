...
if __name__ == "__main__":
    ...
    #
    #     new_arr = []
    #     new_arr.append([olt,ip,gp,onuid,circuit])
    #
    #     worker = mp.cpu_count()
    ...
    #     new_tres.append(a)
        # a["onuid"] = item['onuid']
        # a["circuit"] = item['circuit']
        # a["olt"] = item['olt']
        # a["gp"] = item['gp']
        # a["olt_ip"] = item['olt_ip']

    # data = json.loads(arr_req)
    # arr_req=arr_req.split(",")
    ...
    # cur = con.cursor()
    #
    # sql = "select a.*,b.vendor,b.olt_ip,b.gpon_port,b.onu_id from ( "
    # sql += "select TICKET_NUMBER,TELEPHONE_NUMBER,DIVISION,SUBDIVISION,TEAM,FAULT_TYPE_CD,to_char(APPOINT_DATETIME,'dd/Mon/YYYY hh24:mi:ss') as APPOINT_DATETIME "
    # sql += "from cmtsusr.fault_pending@dbl_nqidb2 where prod_categary=:1 and team = :2  and CATEGARY = :3 and LENGTH(TELEPHONE_NUMBER) = 10)a left join "
    # sql += "(select login,vendor,olt_ip,gpon_port,onu_id from fttx_register_sub)b on substr(a.TELEPHONE_NUMBER,0,10) = b.login"
    #
    # cate = "08 | แจ้งเสีย"
    # prod_cate = '74 | Fiber To Home'
    # cur.execute(sql,{'1':prod_cate,'2':team,'3':cate})
    # tres = cur.fetchall()
    #
    # # close cursor
    # cur.close()
    ...
    # for x in tres:
    #     #VENDOR,OLT_IP,GPON_PORT,ONU_ID,TELEPHONE_NUMBER
    #     new_arr.append([x[7],x[8],x[9],x[10],x[1]])
    #
    # worker = mp.cpu_count()
    # pool = mp.Pool(processes=int(worker*15))
    # res = pool.map(getData,new_arr)
    ...
    #
    #             a["TICKET_NUMBER"] = x[0]
    #             a["TELEPHONE_NUMBER"] = x[1]
    #             a["DIVISION"] = x[2]
    #             a["SUBDIVISION"] = x[3]
    #             a["TEAM"] = x[4]
    #             a["FAULT_TYPE_CD"] = x[5]
    #             a["APPOINT_DATETIME"] = x[6]
    #             a["state"] = y[1]
    #             a["1490RX"] = y[2]
    #             a["1550"] = y[3]
    #             a["1310TX"] = y[4]
